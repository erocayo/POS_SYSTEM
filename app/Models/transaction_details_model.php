...
public function Get_All_Sale_Transaction_Details($SALE_TRANSACTION_ID)
{
   return DB::table('sale_transaction_details as std')
        ...
        ->select('std.*', 'p.PRODUCT_NAME')
        ->where('std.SALE_TRANSACTION_ID', $SALE_TRANSACTION_ID)
        ->get();
}

    public function Set_Sale_Transaction_Details_Entry($SALE_TRANSACTION_ID, $PRODUCT_ID, $UNIT_PRICE, $QUANTITY, $TOTAL){
    return DB::insert(
        'INSERT INTO sale_transaction_details (SALE_TRANSACTION_ID, PRODUCT_ID, UNIT_PRICE, QUANTITY, TOTAL ) 
         VALUES (?,?,?,?,?)',
